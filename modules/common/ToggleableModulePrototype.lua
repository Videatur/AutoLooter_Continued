--[[
    AutoLooter Toggleable Module Prototype
    Mixin for modules that can be toggled on/off based on profile settings.
    Automatically enables or disables the module when its conditions change.
]]
...
function prototype:OnInitialize()
    ...
    local db = self:GetDb() or error("When using this ToggleableModulePrototype you need to implement GetDb() or "
